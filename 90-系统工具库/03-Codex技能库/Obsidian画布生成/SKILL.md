...
description: "Primary Obsidian Canvas（黑曜石画布）creator skill（生成技能） for turning text（文本）, outlines（大纲）, articles（文章）, and notes（笔记） into Canvas（画布） files, mind maps（思维导图）, or spatial diagrams（空间图）。Use for normal Canvas（画布） and mind-map（思维导图） requests."
---
